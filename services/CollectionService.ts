...
class CollectionService {
   ...
   */
  public getCollectionByPageId = async (
    pageID: number,
   ...
   */
  public insertCollectionAndReturnID = async (
    pageID: number,
    templateID: number,
    txn?: SQLite.SQLiteDatabase,
  ): Promise<number> => {
  ...
  public saveCollection = async (
    ...
    try {
      const pageID = await executeTransaction<number | null>(async (txn) => {
        ...
        const collectionID = await this.insertCollectionAndReturnID(
          pageID,
          templateID,
          txn,
        );
