...
// EXAMPLE FLOW
export const saveAndRetrieveCollection = async (collectionDTO: CollectionDTO): Promise<void> => {
    try {
      ...
      if (!templateID) {
        await executeQuery(insertItemTemplate, [collectionDTO.template.template_name]);
        ...
        if (!collectionDTO.template.item_templateID) throw new Error("Template ID could not be retrieved");
